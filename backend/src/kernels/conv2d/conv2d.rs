...
const CONV2D_SOURCE: &str = include_str!("conv2d.comp");

#[derive(Debug, Eq, Hash, PartialEq, Clone)]
pub struct Conv2DPipelineSpec {
    local_x: u32,
    stride_h: u32,
    stride_w: u32,
    dilation_h: u32,
...
impl PushConstSpec for Conv2DPushConst {
...
impl PipelineSpec for Conv2DPipelineSpec {
    type PushConst = Conv2DPushConst;

    fn get_shader(&self) -> CompilationArtifact {
        let mut compiler: compiler::ShaderCompiler =
            compiler::ShaderCompiler::new("conv2d.comp", CONV2D_SOURCE);
...
pub fn run(
    ...
    let pipeline = inst.get_pipeline_from_spec(PipelineSpecs::Conv2D(spec.clone()));
    ...
    if total_elements > chunk_size {
        for in_channels in 0..filters.dims[2] as u32 {
            push.in_channel = in_channels;
            for out_channels in 0..filters.dims[3] as u32 {
                push.out_channel = out_channels;
