...
const RELU_SOURCE: &str = include_str!("relu.comp");

#[derive(Debug, Eq, Hash, PartialEq, Clone)]
pub struct ReluPipelineSpec {
    local_x: u32,
...
#[repr(C, packed)]
pub struct ReluPushConst {
    pub start: u32,
    pub stop: u32,
}

impl PushConstSpec for ReluPushConst {
...
impl PipelineSpec for ReluPipelineSpec {
    type PushConst = ReluPushConst;

    fn get_shader(&self) -> CompilationArtifact {
        let mut compiler: compiler::ShaderCompiler =
            compiler::ShaderCompiler::new("relu.comp", RELU_SOURCE);
...
pub fn run(
    ...
    let spec = ReluPipelineSpec {
        local_x: inst.device_props.sub_group_size.max(1),
        d_type,
    };
    let pipeline = inst.get_pipeline_from_spec(PipelineSpecs::Relu(spec.clone()));
